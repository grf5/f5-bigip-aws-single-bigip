...
alias bigip-pri-az1='ssh admin@${aws_eip.F5_BIGIP_PRI_AZ1EIP_MGMT.public_ip} -p 22 -i "${local_file.newkey_pem.filename}"'
alias bigip-sec-az1='ssh admin@${aws_eip.F5_BIGIP_SEC_AZ1EIP_MGMT.public_ip} -p 22 -i "${local_file.newkey_pem.filename}"'
alias bigip-pri-az2='ssh admin@${aws_eip.F5_BIGIP_PRI_AZ2EIP_MGMT.public_ip} -p 22 -i "${local_file.newkey_pem.filename}"'
alias bigip-sec-az2='ssh admin@${aws_eip.F5_BIGIP_SEC_AZ2EIP_MGMT.public_ip} -p 22 -i "${local_file.newkey_pem.filename}"'
EOT
}
